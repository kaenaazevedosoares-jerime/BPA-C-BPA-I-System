...
const BpaConsolidatedForm: React.FC<BpaConsolidatedFormProps> = ({ onCancel, onSave }) => {
  ...
  const monthsPT = useMemo(() => ['Janeiro', 'Fevereiro', 'Março', 'Abril', 'Maio', 'Junho', 'Julho', 'Agosto', 'Setembro', 'Outubro', 'Novembro', 'Dezembro'], []);

  const recentFilterMonths = useMemo(() => {
    const res: string[] = [];
    const now = new Date();
    for (let i = 0; i < 4; i++) {
  ...
  const getMonthCount = (m: string) => {
    return bpaList.filter(item => item.header.reference_month === m).length;
  };
  ...
  const filteredList = useMemo(() => {
    return bpaList.filter(item => {
      ...
      const matchesProf = filterProf === '' ||
        (item.professionalName && item.professionalName.toLowerCase().includes(filterProf.toLowerCase()));

      return matchesTerm && matchesMonth && matchesProf;
    });
  }, [bpaList, filterTerm, filterMonth, filterProf]);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (profSearch.length >= 3) {
        searchProfessional(profSearch);
      } else {
        setProfSuggestions([]);
      }
    }, 400);
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 pb-32 space-y-6 animate-fade-in">
      ...
      <div className="space-y-4">
        ...
        {/* Smart Filter */}
        <div className="bg-white dark:bg-surface-dark p-4 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-800 space-y-4 animate-fade-in">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="w-full md:w-64 relative">
              ...
              >
                <option value="">Todos os meses ({bpaList.length})</option>
                {recentFilterMonths.map(m => (
                  <option key={m} value={m}>{m} ({getMonthCount(m)})</option>
                ))}
                {/* Fallback for data outside the recent range */}
                {Array.from(new Set(bpaList.map(item => item.header.reference_month)))
                  .filter(m => !recentFilterMonths.includes(m))
                  .sort()
                  .map(m => (
                    <option key={m} value={m}>{m} ({getMonthCount(m)})</option>
                  ))
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center">
            <div className="flex-1 relative w-full">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 material-symbols-outlined text-slate-400 text-[20px]">person_search</span>
              <input
                type="text"
            ...
            {(filterTerm || filterMonth || filterProf) && (
              <button
                onClick={() => { setFilterTerm(''); setFilterMonth(''); setFilterProf(''); }}
                className="w-full md:w-auto px-6 py-2.5 text-slate-500 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-colors text-sm font-bold flex items-center gap-2 justify-center"
              >
